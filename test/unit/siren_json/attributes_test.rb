...
describe HyperResource::Attributes do
  ...
  describe 'accessors' do
    ...
    it "provides access to all attributes" do
      # @attrs.page.must_equal 'error'
      # @attrs.limit.must_equal 'more error'
    end

    it 'supports null attributes' do
      # @attrs.attr3.must_equal 'three error'
    end
  ...
  describe 'changed' do
    before do
      # @rsrc = TestAPI.new(:adapter => HyperResource::Adapter::SIREN_JSON)
      # @rsrc.adapter.apply(SIREN_BODY, @rsrc)
    end
